...
func (r *IncidentRetention) Archive(ctx context.Context, incidentID shared.IncidentID, archivedAt time.Time) error {
	...
	if _, err := tx.Exec(ctx, `INSERT INTO eventsourcing.archived_incidents (incident_id, archived_at, reason) VALUES ($1,$2,'PURGE') ON CONFLICT DO NOTHING`, id, archivedAt); err != nil {
		return fmt.Errorf("archive audit: %w", err)
	}
	if _, err := tx.Exec(ctx, `DELETE FROM eventsourcing.events e USING eventsourcing.aggregate_index i WHERE e.stream_type=i.stream_type AND e.stream_id=i.stream_id AND i.incident_id=$1`, id); err != nil {
