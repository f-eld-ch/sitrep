...
function DefaultAccess() {
  ...
  const defaultAccessResult = useDefaultAccess();
  const usersResult = useAccessUsers();
  const groupsResult = useAccessGroups();
  const [setMode, modeState] = useSetDefaultAccessMode();
  const [grantRole, grantState] = useGrantDefaultRole();
  const [revokeRole, revokeState] = useRevokeDefaultRole();
  ...
  useRedirectIfForbidden(
    defaultAccessResult.status === "error" ? defaultAccessResult.error : undefined,
  );

  if (defaultAccessResult.status === "loading") return <Spinner />;
  if (defaultAccessResult.status === "error") {
    if (defaultAccessResult.error.code === "FORBIDDEN") return null;
    return <Notification variant="danger">{defaultAccessResult.error.message}</Notification>;
  }

  const { mode, grants } = defaultAccessResult.data;
  ...
  const grantedPrincipalKeys = new Set<string>(
    grants.map((g) => `${g.principalKind}:${g.principalId}`),
  );
  ...
  const doRevoke = (
    ...
    void revokeRole({ principalKind: pkind, principalId: pid, role })
      .catch((err: unknown) => {
        setMutationError(err instanceof Error ? err.message : t("defaultAccess.revokeFailed"));
      })
      .finally(() => {
  ...
  const doGrant = (principalId: string, role: IncidentRole) => {
    ...
    void grantRole({ principalKind, principalId, role })
      .catch((err: unknown) => {
        setMutationError(err instanceof Error ? err.message : t("defaultAccess.grantFailed"));
      })
      .finally(() => {
  ...
  const doRoleChange = (
    ...
    void grantRole({ principalKind: pkind, principalId: pid, role })
      .catch((err: unknown) => {
        setMutationError(err instanceof Error ? err.message : t("defaultAccess.roleChangeFailed"));
      })
      .finally(() => {
        setPendingGrants((prev) => {
          const next = new Set(prev);
  ...
  return (
    <>
      <PageTitle level={2}>{t("defaultAccess.title")}</PageTitle>
      <p className="mb-6 text-sm text-fg-muted">{t("defaultAccess.description")}</p>

      {mutationError && (
        <Notification variant="danger" light className="mb-4 flex items-start justify-between">
      ...
      {(modeState.error ?? grantState.error ?? revokeState.error) && (
        <Notification variant="danger" className="mb-4">
          {(modeState.error ?? grantState.error ?? revokeState.error)?.message}
        </Notification>
      )}
        ...
        <div className="space-y-3">
          <label className="flex cursor-pointer items-start gap-2">
            <input
              ...
              onChange={() => {
                if (isRestricted) void setMode({ mode: "OPEN_OPERATIONAL" });
              }}
            />
            <div>
          ...
          <label className="flex cursor-pointer items-start gap-2">
            <input
              ...
              className="mt-0.5 shrink-0"
              onChange={() => {
                if (!isRestricted) void setMode({ mode: "RESTRICTED" });
              }}
            />
            <div>
      ...
      <div className={box}>
        <h4 className="mb-4 text-xl font-bold">{t("defaultAccess.grantsTitle")}</h4>
        <p className="mb-3 text-sm text-fg-muted">{t("defaultAccess.grantsHelp")}</p>

        {grants.length === 0 ? (
          <p className="mb-4 text-sm text-fg-muted">{t("defaultAccess.noGrants")}</p>
        ) : (
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {[...grants]
                  .map((grant) => {
                    const user =
                      grant.principalKind === "USER" ? userBySub.get(grant.principalId) : undefined;
                    const displayName =
                      grant.principalKind === "GROUP"
                        ? groupsResult.status === "ready"
                          ...
                          : grant.principalId
                        : user?.name || user?.email || grant.principalId;
                    ...
                    return (
                      <tr key={principalKey} className="border-b border-border">
                        <td className="py-2 pr-4">
                          <span
                            title={grant.principalKind === "USER" ? grant.principalId : undefined}
                          >
                            {displayName}
                          </span>
