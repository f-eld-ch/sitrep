...
describe("IncidentCard", () => {
    ...
    it("renders incident name and location", () => {
        render(<IncidentCard incident={baseIncident} closeIncident={mockCloseIncident} />);
        expect(screen.getByText("Test Incident")).toBeInTheDocument();
        expect(screen.getByText("Test Location")).toBeInTheDocument();
    });

    it("renders close button when incident is open", () => {
        render(<IncidentCard incident={{ ...baseIncident, closedAt: null }} closeIncident={mockCloseIncident} />);
        expect(screen.getByTestId("close-button")).toBeInTheDocument();
        expect(screen.queryByTestId("open-button")).not.toBeInTheDocument();
    });

    it("renders open button when incident is closed", () => {
        render(<IncidentCard incident={{ ...baseIncident, closedAt: new Date() }} closeIncident={mockCloseIncident} />);
