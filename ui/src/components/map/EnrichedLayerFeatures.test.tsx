...
describe("EnrichedLayerFeatures", () => {
    ...
    it("filters out deleted features and the selected feature", () => {
        const { container } = renderWithMap(
            <EnrichedFeaturesSource id="test" featureCollection={baseFeatureCollection} selectedFeature="line-1" />
        );
        expect(container).toBeTruthy();
    });
    ...
    it("handles empty featureCollection", () => {
        const { container } = renderWithMap(
            <EnrichedFeaturesSource id="test" featureCollection={{ type: "FeatureCollection", features: [] }} />
        );
        expect(container).toBeTruthy();
    });
});
